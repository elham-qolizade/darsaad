...
export default function CategoryListPage({ token }) {
        ...
        categoriesArray.map(async (cat) => {
          try {
            ...
            return {
              ...
              items: filteredProducts.map((p) => p.title || "Unnamed"),
            };
          } catch (err) {
            console.warn(`Error fetching products for category ${cat.id}`, err);
            return {
              ...cat,
              productCount: 0,
              items: [],
  ...
  const handleAddCategory = async (newCategoryData) => {
    try {
      ...
      throw err; // to show error in modal
    }
  };
  ...
  return (
    <div className="min-h-screen relative bg-gray-100 flex flex-col rtl text-right sm:flex-row-reverse">
        <Sidebar />
      <main className="flex-1 p-6 sm:p-8 flex flex-col items-center justify-center sm:items-start sm:justify-start">
        <div className="flex flex-col items-center sm:items-start mb-6">
          ...
          >
              ...
              <span className="text-center">Add New Category</span>
          
          </button>
          <p className="text-gray-600 text-sm mt-1">Total Categories: {totalCount}</p>
        </div>

        <div dir="ltr" className="w-full flex-col max-w-screen-2xl mx-auto px-6">
